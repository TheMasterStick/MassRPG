...
namespace MassRPG.Tests
{
    public sealed class DualWieldEquipmentTests
    {
        ...
        public void TwoOneHandedWeapons_CanOccupyBothHands()
        {
            ...
            Assert.IsTrue(InventoryRules.EquipFromInventory(inventory, equipment, items, 0, EquipmentSlot.MainHand).Success);
            Assert.IsTrue(InventoryRules.EquipFromInventory(inventory, equipment, items, 1, EquipmentSlot.OffHand).Success);
        ...
        public void TwoHandedWeapon_DisplacesOffHandWeapon()
        {
            ...
            Assert.IsTrue(InventoryRules.EquipFromInventory(inventory, equipment, items, 0, EquipmentSlot.MainHand).Success);
            Assert.IsTrue(InventoryRules.EquipFromInventory(inventory, equipment, items, 1, EquipmentSlot.OffHand).Success);
            Assert.IsTrue(InventoryRules.EquipFromInventory(inventory, equipment, items, 2, EquipmentSlot.MainHand).Success);
        ...
        public void ShieldAndOffHandWeapon_RemainMutuallyExclusiveBySharingOffHandSlot()
        {
            ...
            Assert.IsTrue(InventoryRules.EquipFromInventory(inventory, equipment, items, 0, EquipmentSlot.OffHand).Success);
            Assert.IsTrue(InventoryRules.EquipFromInventory(inventory, equipment, items, 1, EquipmentSlot.OffHand).Success);
